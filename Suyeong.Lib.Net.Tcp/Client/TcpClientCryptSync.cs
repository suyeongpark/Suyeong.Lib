...
namespace Suyeong.Lib.Net.Tcp
{
    public class TcpClientCryptSync
    {
        ...
        public TcpClientCryptSync(string ip, int port, byte[] key, byte[] iv)
        {
            this.key = key;
            this.iv = iv;
        ...
        public void Send(ITcpPacket sendPacket, Action<ITcpPacket> callback)
        {
            ...
            try
            {
                using (NetworkStream stream = this.client.GetStream())
                {
                    ...
                    cryptData = StreamUtil.Encrypt(data: sendData, key: this.key, iv: this.iv);
                    ...
                    if (nbytes > 0)
                    {
                        ...
                        // 6. 결과는 압축되어 있으므로 푼다.
                        decryptData = StreamUtil.Decrypt(data: recieveData, key: this.key, iv: this.iv);
