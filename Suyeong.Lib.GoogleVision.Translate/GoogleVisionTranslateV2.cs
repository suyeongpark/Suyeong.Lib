...
namespace Suyeong.Lib.GoogleVision.Translate
{
    ...
    {
        public static string GetTranslate(string text, GoogleLanguage targetLanguage)
        {
            ...
            using (TranslationClient client = TranslationClient.Create())
            {
                ...
                if (LanguageDic.TryGetValue(targetLanguage, out languageStr))
                {
                    TranslationResult response = client.TranslateText(text, languageStr);
